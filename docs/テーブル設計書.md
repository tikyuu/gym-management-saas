...
1行が、1つの事業者に所属する1人のスタッフの業務プロフィールを表す。スタッフ招待時にCognitoのログインアカウントとこのスタッフを同時に作成し、Cognitoが発行した`sub`を持つ`user_accounts`へ紐付ける。招待を承諾する前でも、スタッフとログインアカウントは作成済みとする。

| 列 | 必須 | 意味 |
| --- | --- | --- |
| `id` | はい | スタッフを識別する主キー |
| `organization_id` | はい | 所属する`organizations.id`を指す外部キー |
| `account_id` | はい | 招待時に作成する`user_accounts.id`を指す外部キー |
| `name` | はい | スタッフの氏名 |
| `status` | はい | `invited`、`active`または`inactive` |
| `created_at` | はい | スタッフを登録した日時 |
| `updated_at` | はい | スタッフ情報を最後に更新した日時 |

初期状態は`invited`とする。`invited`はCognitoの招待メールを送信済みだが初回ログインを完了していない状態、`active`は利用中、`inactive`は退職または利用停止中を表す。`inactive`へ変更しても物理削除せず、過去の予約、担当実績および操作履歴を保持する。
