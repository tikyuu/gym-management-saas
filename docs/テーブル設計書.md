...
プランを`inactive`へ変更しても、既存の契約を自動変更または削除しない。料金や利用条件を変更した場合に既存契約へ影響を与えないよう、契約テーブルの設計時に契約時点のプラン条件を保持する。
...
新しい申込みで店舗を選択できるのは、プラン、店舗およびこの設定がすべて`active`の場合だけとする。利用停止時は`status`を`inactive`へ変更し、既存契約や履歴を削除しない。契約時点の利用可能店舗は、後続の`contracts`設計で保持する。
...
新しい予約でメニューを選択できるのは、契約プラン、この設定および選択店舗の`store_menus`がすべて`active`の場合だけとする。利用停止時は`status`を`inactive`へ変更し、既存契約や履歴を削除しない。契約時点の利用可能メニューは、後続の`contracts`設計で保持する。
...
| `plan_snapshot` | はい | 契約時点のプラン名、料金、利用回数、契約期間、利用可能店舗・メニューを保存するJSONB |
...
`plan_snapshot`は契約開始時に固定し、後からプラン、利用可能店舗または利用可能メニューの設定が変わっても、既存契約の条件を変えない。契約期間は`starts_on`から`ends_on`までとし、同一会員について利用期間が重複する契約は登録しない。拒否、停止、終了および利用回数の調整に伴う理由・実行者・変更前後の値は、後続の契約履歴設計で記録する。
...
予約作成時の初期状態は`confirmed`とする。予約作成時には、会員、契約、店舗、メニュー、プラン利用可能店舗・メニュー、店舗提供メニュー、トレーナーの店舗所属および担当可能メニューが有効であることを確認する。`contract_id`は`member_id`の契約であり、予約時間が契約期間内であることも確認する。
...
会員と担当トレーナーの二重予約を防ぐため、`confirmed`の予約に対して、会員ごとおよび`staff_store_membership_id`ごとに`starts_at`から`ends_at`までの時間帯が重複しないPostgreSQLの排他制約を設定する。予約作成はトランザクションで実行する。キャンセル・完了・無断欠席および状態訂正の理由・実行者・変更前後の値は、後続の予約履歴設計で記録する。
...
| `entry_type` | はい | `grant`、`carryover`、`hold`、`release`、`consume`または`adjustment` |
...
契約開始時の付与は`grant`、定期更新時の繰越は`carryover`、管理者による例外的な付与・返却は`adjustment`とする。`adjustment`では`reason`と`executed_by_account_id`を必須にする。履歴は更新・削除せず、誤りは反対方向の調整履歴を追加して訂正する。予約作成と`hold`の記録は同じトランザクションで実行する。
