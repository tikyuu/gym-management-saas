...
契約作成時、`plan_snapshot`へ利用可能店舗のUUID配列`store_ids`と利用可能メニューのUUID配列`menu_ids`を保存する。予約候補の取得ではこの固定済み範囲を使用し、後から変更されたプランの設定で既存契約の利用範囲を拡大・縮小しない。店舗・メニュー自体や店舗での提供設定が停止中の場合は候補から除外する。
...
判定対象日は日本時間とし、通常営業時間より特定日設定を優先する。各営業時間帯の開始時刻を起点に予約開始間隔で候補を作り、終了時刻が同じ営業時間帯に収まるものだけを返す。過去の候補、会員自身の予約と重なる候補、担当トレーナー全員が勤務外・予約不可・予約済みの候補は除外する。契約の利用期間と契約時に固定した店舗・メニューの利用範囲も、指定日で確認する。
...
契約作成時の`plan_snapshot.usage_type`を利用種別の判定に使い、回数制プランで対象期間の付与済み残数がない場合は`409 Conflict`とする。
...
回数制契約では承認と同じトランザクションで初回利用期間の`grant`を記録する。将来開始の`scheduled`契約も期間内の未来予約を受け付けるため、付与履歴を未来の利用期間に紐付けて先行記録する。以降の定期更新時の付与・繰越は別の定期処理で行う。
